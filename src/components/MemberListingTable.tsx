...
export const MemberListingTable: React.FC<MemberListingTableProps> = ({
  ...
  const filteredMembers = useMemo(() => {
    return members.filter(m => {
      ...
      if (filterText.trim()) {
        ...
        if (!matchName && !matchUserId && !matchGroup && !matchLead && !matchPhone && !matchPassport && !matchStatus) {
          return false;
        }
      }
      return true;
  ...
  }, [members, filterDistance, filterGender, filterStatus, filterText, sortField, sortDirection]);
  ...
  const renderStatusBadge = (statusStr?: string) => {
    if (!statusStr || !statusStr.trim()) {
      return <span className="text-neutral-400 font-mono-tech text-[11px]">---</span>;
    }
    const s = statusStr.trim();
    const sUpper = s.toUpperCase();

    // Success / Thành công / Đã thanh toán / Active / Hoàn thành
    if (
      sUpper.includes('SUCCESS') || 
      sUpper.includes('THÀNH CÔNG') || 
      sUpper.includes('THANH TOÁN') || 
      sUpper.includes('ĐÃ') || 
      sUpper.includes('OK') ||
      sUpper.includes('COMPLETE') ||
      sUpper.includes('PAID')
    ) {
      return (
        <span className="inline-flex items-center gap-1 bg-emerald-100 text-emerald-900 border border-emerald-400 px-2 py-0.5 font-bold text-[10px] tracking-wide whitespace-nowrap">
    ...
    if (
      ...
      sUpper.includes('CHỜ') || 
      sUpper.includes('CHƯA') || 
      sUpper.includes('WAITING') ||
      sUpper.includes('PROCESSING')
    ) {
      return (
        <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-900 border border-amber-400 px-2 py-0.5 font-bold text-[10px] tracking-wide whitespace-nowrap">
    ...
    // Failed / Thất bại / Hủy / Cancel / Lỗi
    if (
      sUpper.includes('FAIL') || 
      sUpper.includes('THẤT BẠI') || 
      sUpper.includes('HỦY') || 
      sUpper.includes('CANCEL') ||
      sUpper.includes('ERROR') ||
      sUpper.includes('LỖI')
    ) {
      return (
        <span className="inline-flex items-center gap-1 bg-red-100 text-red-900 border border-red-400 px-2 py-0.5 font-bold text-[10px] tracking-wide whitespace-nowrap">
    ...
    // Default neutral tag
    return (
      <span className="inline-flex items-center gap-1 bg-neutral-100 text-neutral-800 border border-neutral-300 px-2 py-0.5 font-bold text-[10px] tracking-wide whitespace-nowrap">
        <span>{s}</span>
      </span>
    );
  };
  ...
  const handleExportCSV = () => {
    const headers = [
      'STT', 'USER_ID', 'NAME', 'ID_PASSPORT', 'PH_NO', 'DISTANCE', 'GENDER', 'STATUS', 'TXNAMOUNT', 'DATE_CREATE'
    ];
    ...
    <section className="bg-[#F4F1EA] py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="border-2 border-[#1A1A1A] bg-[#F4F1EA] shadow-[4px_4px_0px_0px_#1A1A1A] p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 font-mono-tech text-xs">
            ...
            {/* Filter Status */}
            <div className="flex items-center gap-2 bg-white border border-[#1A1A1A] px-3 py-1.5 min-w-0">
              <span className="text-[11px] font-bold text-neutral-600 uppercase whitespace-nowrap shrink-0">TRẠNG THÁI:</span>
              <select
                value={filterStatus}
                onChange={(e) => {
          ...
          <table className="w-full text-left font-mono-tech text-xs border-collapse">
            ...
            <thead>
              <tr className="bg-[#1A1A1A] text-[#F4F1EA] uppercase tracking-wider text-[11px] border-b-2 border-[#1A1A1A]">
                ...
                <th 
                  ...
                  className="p-3 border-r border-neutral-700 font-bold cursor-pointer hover:bg-neutral-800 select-none text-center min-w-[130px]"
                >
                  <div className="flex items-center justify-center gap-1">
                    <span>STATUS</span>
                    <ArrowUpDown className="w-3 h-3 text-neutral-400" />
            ...
            <tbody>
              {paginatedMembers.length === 0 ? (
                <tr>
                  <td colSpan={10} className="p-8 text-center text-neutral-500 font-mono-tech italic">
